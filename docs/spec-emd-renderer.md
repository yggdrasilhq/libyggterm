...
mapping, extracted from yggterm's shell crate as a pure move. Named
`paper-renderer` at birth, renamed `emd-renderer` (extended markdown) once the
grammar, not the app, was understood to be the thing being specified.

**This spec lives with the crate.** Both moved into `libyggterm` on 2026-08-02
under MPL-2.0, because the engine is a platform organ every pipeline app links
— yedit and ztlkasten's document surfaces, breezed, jyas-webapp — and a
platform library that apps must link is MPL by the estate's licence-by-role
rule, not GPL.

**Owner surfaces:** `crates/emd-renderer` (model/parse), yggterm's shell
...
- `emd-renderer`: model (`MdBlock`/`MdInline`) + parse
  ...
  context).
- yggterm's shell crate keeps the Dioxus RENDER (`md_block_node`,
  `md_inline_nodes`, `document_reading_typography`) for now; extract it as
  `emd-renderer-dioxus` (or a `render-dioxus` feature) into this repo once the
  visual language stops changing weekly. yggterm's `DESIGN.md` §"Document
  reading font" is the style SSOT.
