...
//!
//! Spec: `docs/spec-emd-renderer.md` (this repo). One sentence: source
...
//! - The Dioxus RENDER of these blocks still lives in yggterm's shell crate
//!   (`md_block_node` / `md_inline_nodes` + `document_reading_typography`);
//!   extracting it into libyggterm is the spec's next seam, once the render
...
/// the substrate for block click-to-edit (spec §3, the round-trip invariant):
